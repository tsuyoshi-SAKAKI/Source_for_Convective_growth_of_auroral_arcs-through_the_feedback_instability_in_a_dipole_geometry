program kenkyuu
 ...
 real(8),dimension(-nx:nx ,0:ny ,-nz:nz)  :: ig11, ig22, ig33, g33, h1, h2, h3, dcx, dcy,dcz, b0, jcb,ig13
 ...
 x0 = (sin(pi/9.d0))**2         
 mu0 = -cos(pi/9.d0) / sqrt(1.d0 - x0)     
 lx       = -(sin(pi/9.d0 - bound))**2 +(sin(pi/9.d0 + bound))**2  
 lz       = -log(alp * mu0 + sqrt(1.d0 + (alp * mu0)**2)) / ifalp  
...
! write(*,*)"#dx= " ,dx
 ...
 do iz = -nz, nz
  ...
  zeta(mx,iz) = (mu(iz) ** 2 * (1.d0 - xi1(mx))) / (xi1(mx) ** 4 ) 
  ...
  ig33(mx,my,iz) = dxdm(mx,iz) **2 * (1.d0 / radius(mx,iz)**6 / (1.d0 - xi1(mx))**3) *    &
                   ((cos(theta(mx,iz)) * (1.d0 + 3.d0*(1.d0 - xi1(mx))))**2 / 4.d0   &
                   + usi(mx,iz) * (1.d0 - (1.d0 /radius(mx,iz)))**2)
  ig13(mx,my,iz) = dxdm(mx,iz) / radius(mx,iz) **5.d0 * (sin(theta0(mx)))**2.d0 * cos(theta(mx,iz)) &
					/ 2.d0 / (cos(theta0(mx)))**3 * capth(mx,iz)

  g33(mx,my,iz)  = radius(mx,iz) ** 6 * (1.d0 - xi1(mx)) / dxdm(mx,iz) ** 2 / capth(mx,iz)
 
  jcb(mx,my,iz) = radius(mx,iz) ** 6 * (1.d0 - xi1(mx)) ** 0.5d0 / capth(mx,iz) / dxdm(mx,iz)
