...
!$OMP parallel workshare
        ...
        dpsidz(:,:,:) =-0.5_DP * ( dudz(:,:,:) - dddz(:,:,:) )
!$OMP end parallel workshare

      call fld_lapl( dpsidz, dcprdz, 2*nz )

!--- up-wind

      call fld_lapl( omg, lpo, 2*nz )
!Before
!      do iz = -nz, nz-1
!        do my = 0, ny
!          do ix = -nx, nx
!            domg(ix,my,iz) = (     -psnb1(ix,my,iz)                       &
!                   + valf(iz)**2 * ( psnb2(ix,my,iz) + dcprdz(ix,my,iz) ) &
!                   + nu  * lpo(ix,my,iz) ) * dt
!            dpsi(ix,my,iz) = (                                            &
!                   +               ( psnb3(ix,my,iz) + dphidz(ix,my,iz) ) &
!                   + eta * cpr(ix,my,iz) ) * dt
...
!$OMP parallel do collapse(2) private(my,mx)
      do my = 0, ny
        do mx = -nx, nx
          didns(mx,my) = ( icpr(mx,my) - 2.d+0 * alpha * idns(mx,my) &
                       - ipsnb(mx,my) ) * dt
        end do
      end do
