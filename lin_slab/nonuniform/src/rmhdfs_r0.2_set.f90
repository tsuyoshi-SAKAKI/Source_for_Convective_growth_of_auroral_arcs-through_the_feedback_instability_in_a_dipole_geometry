...
!--------------------------------------
    ...
    character(128) :: memo
    character(512) :: f_log, f_hst, f_fld, f_cnt, f_test, f_ll!new
    ...
    namelist /run_n/ inum
!    namelist /files/ f_log, f_hst, f_fld, f_cnt, f_test, f_ll!new
...
!      open( otest, file=trim(f_test)//"g"//crank//".test."//cnew )!new
!      open( oll, file=trim(f_ll)//"g"//crank//".ll."//cnew )!new
...
!          ll(mx)  = 2._DP * lz
          ...
          vaa(mx) = 2._DP*lz/ll(mx)      ! nonuniform field line length
        end do
